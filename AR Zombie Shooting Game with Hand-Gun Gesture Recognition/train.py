...
def make_train_val_indices(targets):
    """전체 데이터를 클래스별로 8:2 비율에 가깝게 train/validation으로 나눈다.

    데이터가 적을 때 완전 랜덤으로 나누면 validation set에 특정 클래스가 너무
    적게 들어갈 수 있다. 그래서 gun/non_gun 각각에서 따로 나눈 뒤 합친다.
    """
    ...
    for class_indices in indices_by_class.values():
        ...
        shuffled_indices = [class_indices[i] for i in order]

        # 클래스별 데이터가 2장 이상이면 validation에 최소 1장은 넣는다.
        if len(shuffled_indices) >= 2:
            val_size = max(1, int(len(shuffled_indices) * 0.2))
        else:
            val_size = 0

        train_indices.extend(shuffled_indices[:-val_size] if val_size > 0 else shuffled_indices)
        val_indices.extend(shuffled_indices[-val_size:] if val_size > 0 else [])

    # train/validation 내부 순서도 한 번 섞어준다.
...
def make_class_weights(targets, num_classes, device):
    """클래스 개수가 다를 때 loss에 사용할 가중치를 만든다."""
    target_tensor = torch.tensor(targets, dtype=torch.long)
    counts = torch.bincount(target_tensor, minlength=num_classes).float()
    total_count = counts.sum()

    # 이미지가 적은 클래스에 더 큰 가중치를 주어 학습 균형을 맞춘다.
...
def main():
    ...
    train_indices, val_indices = make_train_val_indices(base_dataset.targets)
