...
너무 큰 용량의 학습데이터는 업로드가 되기 않기에 실제 학습 이미지와 `dataset_archive/` 폴더는 GitHub에 업로드하지 않는다. 저장소에는 폴더 구조만 유지하고, 필요한 경우 `py collect_data.py` 또는 `py game.py`의 `d` 옵션으로 데이터를 다시 수집한다.
